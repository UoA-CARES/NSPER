...
class NSPER_TD3:
    ...
    def train_policy(self, replay_buffer, batch_size):
        ...
        states, actions, rewards, next_states, dones, novelty_surprises, indices, weights = replay_buffer.sample(batch_size)
        ...
        novelty_surprises     = torch.FloatTensor(np.asarray(novelty_surprises)).to(self.device)
        #indices     = torch.FloatTensor(np.asarray(indices)).to(self.device)
       ...
       # print("Shape of q_values_one", q_values_one.type)

        #print("Shape of q_target:", q_target.shape)

        td_loss1 = (q_values_one - q_target).abs()
        td_loss2 = (q_values_two - q_target).abs()

        # Compute critic loss
        critic_loss = (
                (weights * F.mse_loss(q_values_one, q_target, reduction='none')).mean()
